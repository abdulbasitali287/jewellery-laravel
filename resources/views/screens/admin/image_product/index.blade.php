                                                ...
                                                @foreach($image_product as $image_products)
                                                    <tr class="{{ $loop->iteration % 2 == 0 ? 'even' : 'odd' }}">
                                                        @foreach ($product as $id => $name)
                                                            <td class="dtr-control sorting_1" tabindex="0">{{ $image_products->product_id == $id ? $name : '' }}</td>
                                                        @endforeach


                                                        <td><img src="{{ asset($image_products->image) }}" width="100px"alt=""></td>


                                                        <td>
                                                            <div class="d-flex">
                                                                <a href="{{ route('image.product.edit', $image_products->id) }}"
                                                                    class="btn btn-sm btn-success mx-2">Edit</a>
                                                                {!! Form::open(['url' => url('admin/image-product/destroy/') . '/' . $image_products->id]) !!}
